...
**txstart**   ---calls {gff} --genome {gff} --output {gff}  [--window (int) --open_thresh (int)  --cores (int) --p1.max.merge (int) --p1.max.downstream (int) --max.uncovered (int) ]
	...
	Distance between nucleosomes -1 and +1 to discriminate between 'open' and 'close' classes. Default 215
		--cores [int, 1]
  	Number of computer threads. Optional
		--p1.max.merge [int, 3]. Optional
		--p1.max.downstream [int, 20]. Optional
		--max.uncovered [int, 150]. Optional
**periodicity** --calls {gff} --reads {RData} --type (single|paired) --gffOutput {gff} --bwOutput {bw} --genes {gff} --chrom_sizes {chrom.sizes} [--periodicity (int) --cores (int) ]
...
**stiffness** --calls {gff} --reads {RData} --output {gff} --range [str, All|chr|chr:start-end] [--t [double, 310.15] ]
 	...
 	Output stiffness in GFF format
		--range
 	Genomic range to consider: All: all chromosomes, chr: a single chomosome, chr:start-end range indicated by coordinates. Default All
		--t
  	Temperature (K). Optional

**nucDyn**  --input1 {RData} --input2 {RData} --calls1 {gff} --calls2 {gff} --outputGff {gff} --outputBigWig {bw}  --genome {chrom.sizes} --range (All|chr|chr:start-end) [ --plotRData {RData} --maxDiff (int, 70) --maxLen (int, 140) --shift_min_nreads [int, 3] --shift_threshold [float, 0.1] --indel_min_nreads [int, 3] --indel_threshold [float, 0.05] --cores [int, 1] --equal_size [logical, FALSE] --readSize [int, 140] --roundPow [logical] --same_magnitude [logical] ]
